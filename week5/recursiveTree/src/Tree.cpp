...
//  recursiveTree
//
//  Created by William Enders on 2/29/16.
//
//
...
void Tree:: seed1(float dotSize, float angle, float x, float y){
    ...
    ofFill();
    if(dotSize > 1.0){
        float r = ofRandomuf(); // returns random # between 0,1
        if(r > 0.02){
            // this happens 98% of time
